...
public:
    ...
    bool start(OnDeviceDiscovered on_discovered, OnDeviceLost on_lost) {
        ...
        if (!socket_->bind(local_addr)) {
            LOG_ERROR("DiscoveryEngine", std::string("Failed to bind to discovery port ") + std::to_string(DISCOVERY_PORT));
            return false;
        }
...
private:
    void receive_loop() {
        ...
        LOG_INFO("DiscoveryEngine", std::string("Receive loop started - listening on port ") + std::to_string(DISCOVERY_PORT));
        ...
        while (running_) {
            ...
            LOG_INFO("DiscoveryEngine", std::string("Received ") + std::to_string(received) + " bytes from " + from_addr.ip);
            ...
            if (!device_info.id.empty() && device_info.id == local_device_id_) {
                LOG_INFO("DiscoveryEngine", std::string("Ignoring discovery from self: ") + device_info.name);
                continue;
            }
            ...
            if (!device_info.id.empty()) {
                if (DeviceManager::instance().add_device(device_info)) {
                    ...
                    LOG_INFO("DiscoveryEngine", std::string("Device discovered: ") + device_info.name + " from " + device_info.ip_address);
                }
            } else {
                LOG_INFO("DiscoveryEngine", std::string("Failed to parse discovery message from ") + from_addr.ip);
            }
        }
    }
    ...
    void broadcast_loop() {
        ...
        LOG_INFO("DiscoveryEngine", std::string("Broadcast info - Hostname: ") + hostname + ", IP: " + local_ip + ", MAC: " + mac_address);
        ...
        if (last_dot != std::string::npos) {
            ...
            LOG_INFO("DiscoveryEngine", std::string("Calculated subnet broadcast: ") + subnet_broadcast);
        }
        ...
        while (running_) {
            ...
            for (const auto& addr : broadcast_addrs) {
                ...
                LOG_INFO("DiscoveryEngine", std::string("Broadcasted ") + std::to_string(sent) + " bytes to " + addr + ":" + std::to_string(DISCOVERY_PORT));
            }
    ...
    DeviceInfo parse_discovery_message(const std::vector<uint8_t>& data,
        ...
        if (!reader.read_uint32(magic) || magic != HANDSHAKE_MAGIC) {
            LOG_INFO("DiscoveryEngine", std::string("Invalid magic number from ") + from_addr.ip + ": 0x" + std::to_string(magic));
            return device;  // Invalid magic
        }
        
        if (!reader.read_uint32(version) || version != PROTOCOL_VERSION) {
            LOG_INFO("DiscoveryEngine", std::string("Invalid protocol version from ") + from_addr.ip + ": " + std::to_string(version));
        ...
        if (!reader.read_string(device.id) ||
            ...
            LOG_INFO("DiscoveryEngine", std::string("Failed to parse device info from ") + from_addr.ip);
            return device;  // Failed to parse
        }
        
        uint8_t platform_byte;
        if (!reader.read_uint8(platform_byte)) {
            LOG_INFO("DiscoveryEngine", std::string("Failed to parse platform from ") + from_addr.ip);
            return device;
        }
        ...
        if (!reader.read_string(ip)) {
            LOG_INFO("DiscoveryEngine", std::string("Failed to parse IP address from ") + from_addr.ip);
            return device;
        }
        ...
        if (!reader.read_uint16(device.port) ||
            ...
            LOG_INFO("DiscoveryEngine", std::string("Failed to parse port/version from ") + from_addr.ip);
            return device;
        }
        
        device.last_seen = std::chrono::system_clock::now();
        LOG_INFO("DiscoveryEngine", std::string("Successfully parsed device: ") + device.name + " from " + from_addr.ip);
